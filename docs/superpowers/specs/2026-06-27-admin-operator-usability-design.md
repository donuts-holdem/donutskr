...
4. **설명 편집:** **구조화 블록 에디터**(§Phase 5 — 손실 방지 장치 포함). ⚠️ 두 전문가 리뷰 모두 "마크다운 에디터 + 붙여넣기 정제 + 미리보기"가 동일 문제를 훨씬 낮은 위험으로 해결한다고 지적(§5.1). 블록 방향을 유지하되, 사용자 spec 리뷰에서 재고 여지를 남김.
...
- **블록 모델 확장(리뷰 Critical/HIGH — 손실 방지):** `heading | paragraph | list | image | raw` 등. 문단은 인라인 마크(굵게)·링크 보존, 리스트 블록 포함, 이미지 블록은 **alt 텍스트 필수(WCAG 1.1.1)**, 변환 불가 콘텐츠를 담는 **`raw`(정제 HTML) 이스케이프 블록** 제공. 실제 마이그레이션 전, 기존 프로그램 설명에 쓰인 태그 인벤토리부터 수집.
...
### 5.1 대안: Phase 5를 마크다운 에디터로 (전문가 권고)
두 리뷰어 모두 프로그램 설명을 **마크다운 에디터 + 붙여넣기 정제 + 라이브 미리보기**로 하면 굵게/링크/리스트를 그대로 보존하면서 **jsonb 컬럼·마이그레이션·공개 렌더러 재작성이 불필요**하고(기존 `marked`+`sanitizeHtml` 재사용) 위험이 훨씬 낮다고 평가. L3의 근본 원인은 "정제·미리보기·툴바 없이 raw HTML이 붙여넣어진 것"이라 이 접근으로도 해결됨. 사용자는 구조화 블록을 선택했으므로 본 spec은 블록 경로(손실 방지 포함)를 유지하되, spec 리뷰에서 재고할 수 있도록 명시.
...
| 구조화 블록 마이그레이션으로 기존 설명 손실 | 태그 인벤토리 선행, 블록 모델 확장 + `raw` 이스케이프, 빈 블록 레거시 폴백, 전수 비교 |
