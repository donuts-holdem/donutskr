...
export default async function MemberHomePage() {
  ...
  return <div className="space-y-12">
    <section aria-labelledby="home-heading" className="border-b border-border pb-10">
      <div className="flex flex-wrap items-center justify-between gap-4"><p className="text-xs font-semibold tracking-widest text-gold">YOUR MEMBERSHIP</p><span className="rounded-pill border border-gold/30 px-3 py-1 text-xs text-gold">승인 회원</span></div>
      <h1 id="home-heading" className="mt-6 text-4xl font-bold leading-tight tracking-tight sm:text-5xl">{member.profile.name}님,<br />도너츠에서 만나요.</h1>
      <p className="mt-5 text-sm text-ink/60">{member.school}{member.clubs.length > 0 ? ` / ${member.clubs.map(club => club.name).join(", ")}` : ""}</p>
      {member.isLeader && <Link href="/leader/approvals" className="mt-6 inline-flex min-h-11 items-center gap-3 text-sm font-semibold text-gold focus-visible:outline-2 focus-visible:outline-gold">담당 소속 가입 승인<ArrowUpRight className="size-4" aria-hidden="true" /></Link>}
    </section>
    <section aria-labelledby="my-class-heading">
      ...
      {member.classes.length ? <ul className="mt-5 divide-y divide-border">{member.classes.map(item => <li key={item.id} className="flex flex-col justify-between gap-4 border-l-2 border-gold py-4 pl-5 sm:flex-row sm:items-center"><h3 className="font-semibold">{item.name}</h3><div className="text-sm leading-relaxed text-ink/70"><p>{weekdays[item.weekday]}요일 / {item.start_time.slice(0, 5)}</p><p className="mt-1">{item.place}</p></div></li>)}</ul> : <p className="mt-5 text-sm text-ink/60">등록된 클래스 소속이 없습니다. 운영진에게 문의해 주세요.</p>}
    </section>
