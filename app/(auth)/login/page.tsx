...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ notice?: string }> }) {
  ...
  return (
    <section aria-labelledby="login-heading">
      ...
      <p className="mb-8 mt-3 text-sm leading-relaxed text-ink/60">가입할 때 정한 아이디로 로그인하세요. 같은 사이트에서 클래스·클럽·모임·학습과 기존 일정·시리즈를 함께 이용할 수 있습니다.</p>
      ...
      <div className="mt-6 border-t border-border pt-6">
        <p className="text-sm leading-relaxed text-ink/70">기존 관리자 계정은 이메일로 로그인하세요. 로그인 상태는 회원 화면과 공유됩니다.</p>
        <Link href="/admin/login" className="mt-3 inline-flex min-h-11 items-center text-sm font-semibold text-gold underline-offset-4 hover:underline focus-visible:outline-2 focus-visible:outline-gold">관리자 로그인</Link>
      </div>
    </section>
  );
}
