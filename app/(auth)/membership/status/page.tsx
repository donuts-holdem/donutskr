...
export default async function MembershipStatusPage() {
  ...
  return <section aria-labelledby="status-heading">
    ...
    <p className="text-sm leading-relaxed text-ink/70">{blocked ? "이메일을 다시 인증해도 정지·탈퇴 상태는 해제되지 않습니다." : user.email_confirmed_at ? "이메일 인증이 완료되었습니다. 회원 상태를 새로고침해 주세요." : "가입 메일에서 인증을 완료해 주세요. 클래스·클럽은 별도 승인이 필요합니다."}</p>
