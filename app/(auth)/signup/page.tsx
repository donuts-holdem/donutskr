...
import { getMembershipSession, getOwnApplication, getSignupCatalog } from "@/lib/membership/server";
import { SignupForm } from "@/components/membership/SignupForm";

export const metadata = { title: "현장 가입 신청 | DO:NUTS CLASS" };

export default async function SignupPage() {
  const { supabase, user, profile } = await getMembershipSession();
  if (profile && profile.status !== "PENDING") redirect("/membership/status");
  if (profile && (await getOwnApplication())?.status === "PENDING") redirect("/membership/status");
  if (user) {
    const { data: admin, error } = await supabase.rpc("is_admin");
    if (error) throw new Error("가입 권한을 확인하지 못했습니다.");
    if (admin === true) return <section><h1 className="text-2xl font-bold">운영자 계정</h1><p className="mt-4 text-sm leading-relaxed text-ink/70">운영자 계정과 회원 계정은 분리해 사용합니다.</p><Link href="/admin" className="mt-6 inline-flex py-3 text-gold underline">관리자 화면으로</Link></section>;
  }
