...
export function AnswerReview({ answer }: { answer: GradedAnswer }) {
  ...
  return <article className="border-b border-border py-7">
    <p className="text-xs text-ink/60">{answer.day ? answer.day + " / " : ""}{difficultyLabels[answer.difficulty]} / {answer.answer_mode === "MULTIPLE" ? "복수 정답" : "단일 정답"}</p>
    <h3 className="mt-3 whitespace-pre-wrap text-lg font-semibold leading-relaxed">{answer.prompt}</h3>
    <PokerSpot spot={answer.spot} />
    <p className="mt-4 text-sm font-semibold text-gold">정답: {feedback.correct.join(" / ")}</p>
    {feedback.missing.length > 0 && <p className="mt-2 text-sm text-coral-to">빠뜨린 정답: {feedback.missing.join(" / ")}</p>}
    {feedback.extra.length > 0 && <p className="mt-2 text-sm text-coral-to">잘못 추가한 선택: {feedback.extra.join(" / ")}</p>}
