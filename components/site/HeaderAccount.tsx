...
export async function HeaderAccount() {
  ...
  const assignments = active ? await getLeaderAssignments(session.supabase, session.user.id).catch(() => null) : null;
  const canSwitch = session.isAdmin || Boolean(assignments?.classIds.length || assignments?.clubIds.length);

  return <div className="flex shrink-0 items-center gap-2">
    {canSwitch && <Link href="/mode" className="inline-flex min-h-11 items-center px-2 text-xs font-semibold text-gold focus-visible:outline-2 focus-visible:outline-gold">모드</Link>}
