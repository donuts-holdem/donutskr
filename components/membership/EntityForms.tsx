...
function CreationLeaders({ candidates, currentUserId, prefix }: { candidates: LeaderCandidate[]; currentUserId: string; prefix: string }) {
  return <fieldset className="space-y-3"><legend className="mb-2 text-sm font-semibold">담당 리더 · 최소 1명</legend>
    <LeaderPicker candidates={candidates} prefix={prefix} name="leader_ids" multiple defaultSelected={[currentUserId]} />
  </fieldset>;
}

export function ClassEditor({ course, candidates, currentUserId, existingNames = [] }: { course?: ClassRecord; candidates: LeaderCandidate[]; currentUserId: string; existingNames?: string[] }) {
  ...
  return <ActionForm key={`${prefix}:${course?.revision ?? 0}`} action={saveOperatingEntity} label={course ? "클래스 기본 정보 저장" : "클래스 생성"} disabled={!course && !candidates.length}>
    ...
    <ClassNameFields prefix={prefix} course={course} existingNames={existingNames} />
    ...
    {course ? <><p className="text-sm leading-relaxed text-muted-foreground">등록된 회차 날짜는 회차 상세에서 변경해 주세요.</p><ReasonField id={`${prefix}-reason`} /></> : <><CreationLeaders candidates={candidates} currentUserId={currentUserId} prefix={`${prefix}-leader`} /><ClassCreationDates /></>}
  </ActionForm>;
}
...
export function ClubEditor({ club, schools, candidates, currentUserId }: { club?: ClubRecord; schools: School[]; candidates: LeaderCandidate[]; currentUserId: string }) {
  ...
  return <ActionForm key={`${prefix}:${club?.revision ?? 0}`} action={saveOperatingEntity} label={club ? "클럽 기본 정보 저장" : "클럽 생성"} disabled={!options.length || (!club && !candidates.length)}>
    ...
    <ImageUploadField id={`${prefix}-logo`} name="logo_url" label="클럽 로고 · 선택" initialUrl={club?.logo_url} />
