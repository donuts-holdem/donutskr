  ...
  return <div className="space-y-10">
    ...
    <section aria-labelledby={`${id}-members`}>
      ...
      {!people.members.length ? <p className="mt-5 text-sm text-muted-foreground">승인된 소속 회원이 없습니다. 리더 지정은 회원 가입 승인을 대신하지 않습니다.</p> : <ul className="mt-3 divide-y divide-border">{people.members.map(member => <li key={member.id} className="py-5">
        <div className="flex flex-wrap items-baseline justify-between gap-2"><h3 className="font-medium">{member.name} <span className="text-sm font-normal text-muted-foreground">{member.username}</span></h3><span className="text-xs text-muted-foreground">{!member.active ? "이전 소속" : member.status === "SUSPENDED" ? "소속 유지 · 이용 정지" : member.status === "WITHDRAWN" ? "탈퇴" : "소속 중"}</span></div>
