...
export async function ClubManagement({ id, basePath }: { id: string; basePath: string }) {
  ...
  const { club } = data;
  return <div className="mx-auto max-w-5xl space-y-10"><header className="border-b border-border pb-8"><Button asChild variant="link" className="mb-4 h-11 px-0"><Link href={basePath}>클럽 운영 목록</Link></Button><div className="flex items-start gap-5"><ClubLogo name={club.name} logoUrl={club.logo_url} /><div className="min-w-0"><p className="text-xs font-semibold tracking-widest text-gold">{club.archived_at ? "보관됨" : "운영 중"} / CLUB</p><h1 className="mt-3 break-words text-3xl font-semibold">{club.name}</h1><p className="mt-4 text-sm text-muted-foreground">{data.school} · 회원 {data.memberCount.toLocaleString("ko-KR")}명{club.default_atc !== null ? ` / 기본 ATC ${club.default_atc.toLocaleString("ko-KR")}` : ""}</p></div></div><p className="mt-5 whitespace-pre-wrap break-words text-sm leading-relaxed text-muted-foreground">{club.description || "등록된 클럽 소개가 없습니다."}</p>{club.archived_at && <p className="mt-4 text-sm text-gold">보관된 클럽입니다. 소속과 운영 이력은 읽기 전용으로 유지됩니다.</p>}<div className="mt-4 flex flex-wrap gap-5"><Button asChild variant="outline" className="h-11"><Link href={data.isAdmin ? "/admin/meetings" : "/leader/meetings"}>모임 운영</Link></Button><Button asChild variant="link" className="h-11 px-0"><Link href={`/meetings?club=${id}`}>클럽 모임 보기</Link></Button></div></header>
