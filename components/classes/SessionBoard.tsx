...
import { SessionContentForm } from "@/components/classes/SessionContentForm";
...
export async function SessionBoard({ classId, sessionId, basePath }: { classId: string; sessionId: string; basePath: string }) {
  ...
  return <div className="mx-auto max-w-5xl space-y-10">
    <header className="border-b border-border pb-8"><Button asChild variant="link" className="mb-4 h-11 px-0"><Link href={`${basePath}/${classId}`}>{course.name} 운영으로</Link></Button><p className="text-xs font-semibold tracking-widest text-gold">SESSION {String(session.session_number).padStart(2, "0")} / {sessionStatus(session)}</p><h1 className="mt-4 break-words text-3xl font-semibold">{session.title || `${session.session_number}회차`}</h1><p className="mt-4 text-sm text-muted-foreground"><time dateTime={session.scheduled_at}>{formatSessionDate(session.scheduled_at)}</time> / 한국 시간</p><p className="mt-2 text-sm text-muted-foreground">{course.place}</p>{archived && <p className="mt-4 text-sm text-gold">보관된 클래스입니다. 회차와 출석은 읽기 전용입니다.</p>}{session.cancelled_at && <p className="mt-4 whitespace-pre-wrap break-words text-sm text-gold">취소 사유: {session.cancellation_reason}</p>}<Link href={`/class/${classId}/sessions/${sessionId}`} className="mt-3 inline-flex min-h-11 items-center text-sm text-gold underline underline-offset-4 focus-visible:outline-2 focus-visible:outline-ring">회원 화면 보기</Link></header>
    <section className="space-y-5" aria-labelledby="session-content-heading"><h2 id="session-content-heading" className="text-xl font-semibold">수업 내용</h2><p className="whitespace-pre-wrap break-words text-sm leading-relaxed text-muted-foreground">{session.description || "수업 내용이 아직 등록되지 않았습니다."}</p>{!archived && <details className="rounded-lg border border-border p-5"><summary className="cursor-pointer py-2 text-sm font-semibold focus-visible:outline-2 focus-visible:outline-ring">수업 제목 · 내용 편집</summary><div className="mt-5"><SessionContentForm session={session} /></div></details>}</section>
    <div className="grid items-start gap-10 lg:grid-cols-3">
      <section className="min-w-0 space-y-6 lg:col-span-2" aria-labelledby="attendance-heading">
        <div><h2 id="attendance-heading" className="text-xl font-semibold">{session.status === "SCHEDULED" && session.roster_run > 0 ? "이전 시작 시점의 명단" : "출석 명단"}</h2></div>
