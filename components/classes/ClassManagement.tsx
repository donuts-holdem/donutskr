...
export async function ClassManagementIndex({ basePath }: { basePath: string }) {
  ...
  return <div className="mx-auto max-w-5xl space-y-10">
    <header className="border-b border-border pb-8">{!data.isAdmin && <Link href="/leader?view=class" className="mb-5 inline-flex min-h-11 items-center text-sm text-gold underline underline-offset-4 focus-visible:outline-2 focus-visible:outline-ring">리더 홈</Link>}<p className="text-xs font-semibold tracking-widest text-gold">CLASS OPERATIONS</p><h1 className="mt-4 text-3xl font-semibold">{data.isAdmin ? "클래스 관리" : "담당 클래스"}</h1><Link href={data.isAdmin ? "/admin/members" : "/leader/approvals"} className="mt-4 inline-flex min-h-11 items-center text-sm text-gold underline underline-offset-4 focus-visible:outline-2 focus-visible:outline-ring">소속 신청 승인</Link></header>
